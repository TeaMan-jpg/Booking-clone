...
export const useLocalStorage = (key: string,key2:string,key3:string,key4:string) => {  
    ...
    const setItem3 = (value: unknown) => {
        if (typeof window !== 'undefined') {
             try {
                 window.localStorage.setItem(key4,JSON.stringify(value))
             }
                catch(err) {
                    console.log(err)
                }
    ...
    const getItem4 = () => {
        if (typeof window !== 'undefined') {
            try {
                const item = window.localStorage.getItem(key4)
                return item ? JSON.parse(item) : undefined
            }
            catch(err) {
                console.log(err)
    ...
    return {setItem,getItem,getItem2,removeItem,removeItem2,getItem3,removeItem3,setItem2,setItem3,getItem4,removeItem4}
}